...
from src.router_service.helpers.price_helpers import to_int_percent, int_to_percent, get_price_mod_for
...
class Pricer:
    def __init__(self, price_model: PriceModel):
        ...
        for price in price_model:
            match price.priceTitle:
                ...
                case "highway":
                    self.highway_lookup[price.valueName] = to_int_percent(price.valueDescription)
                case "boundary":
                    self.boundary_lookup[price.valueName] = to_int_percent(price.valueDescription)
                case "vehicleClassification":
                    self.vehicle_classification_lookup[price.valueName] = to_int_percent(price.valueDescription)
                case "fuelType":
                    self.fuel_type_lookup[price.valueName] = to_int_percent(price.valueDescription)
                case "rushPrice":
                    self.rush_lookup[price.valueName] = to_int_percent(price.valueDescription)
    ...
    def calculate_price(self, route: Route, vehicle: Vehicle):
        ...
        """
        vehicle_classification_mod = get_price_mod_for([vehicle.classification],
                                                       self.vehicle_classification_lookup)
        ...
        for segment in route.segments:
            ...
            total_mod = int_to_percent(highway_mod + vehicle_classification_mod + fuel_type_mod + rush_hour_mod)
            # Calculations use cents, end result want euro's
            segment.price = round(self.base_road_price * distance * (total_mod + 1), 2) / 100
